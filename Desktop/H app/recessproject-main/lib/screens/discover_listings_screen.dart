...
import 'package:housingapp/widgets/simple_map_view.dart';
...
class _DiscoverListingsScreenState extends State<DiscoverListingsScreen> {
  ...
  // State variable to track view mode (list or map)
  ...
  Future<void> _showFilterModal(UserPreferences userPreferences) async {
    ...
    if (appliedFilters != null) {
      ...
      userPreferences.updateBudgetRange(
          min: _appliedMinBudget, max: _appliedMaxBudget);
  ...
  List<Property> _applyClientSideFiltersAndScore(
      List<Property> properties, UserPreferences userPreferences) {
    ...
    if (_filtersApplied && properties.isNotEmpty) {
      currentFilteredProperties = currentFilteredProperties.where((property) {
        ...
        bool locationFilterAppliedByUser = (_appliedLocationFilter != null &&
            _appliedLocationFilter!.isNotEmpty);
        ...
        if (locationFilterAppliedByUser) {
          if (property.location
              .toLowerCase()
              .contains(_appliedLocationFilter!.toLowerCase())) {
            matchedFilterCount++;
        ...
        if (_appliedMinBudget != null || _appliedMaxBudget != null) {
          ...
          if (_appliedMinBudget != null &&
              property.price < _appliedMinBudget!) {
            budgetMeetsCriteria = false;
          }
          if (_appliedMaxBudget != null &&
              property.price > _appliedMaxBudget!) {
            budgetMeetsCriteria = false;
          }
        ...
        if (userPreferences.housingType == 'permanent') {
          if (_appliedHouseType != null &&
              property.houseType != null &&
              _appliedHouseType!.toLowerCase() ==
                  property.houseType!.toLowerCase()) {
            matchedFilterCount++;
          }
        } else if (userPreferences.housingType == 'rental') {
          if (_appliedSelfContained != null &&
              property.selfContained != null &&
              _appliedSelfContained! == property.selfContained!) {
            matchedFilterCount++;
          }
          if (_appliedFenced != null &&
              property.fenced != null &&
            ...
            _appliedAmenities.forEach((amenityKey, isSelected) {
              if (isSelected &&
                  (property.amenities != null &&
                      property.amenities![amenityKey] == true)) {
    ...
    if (_appliedReferenceLatitude != null &&
      ...
      for (int i = 0; i < currentFilteredProperties.length; i++) {
        currentFilteredProperties[i] =
            currentFilteredProperties[i].copyWith(distanceKm: null);
      }
    }
    ...
    if (_filtersApplied) {
      for (var property in currentFilteredProperties) {
        double score =
            MatchingAlgorithm.calculateMatchScore(userPreferences, property);
        propertiesToScoreOrNullify.add(property.copyWith(matchScore: score));
      }
      propertiesToScoreOrNullify
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
            '${userPreferences.housingType![0].toUpperCase() + userPreferences.housingType!.substring(1)} Listings'),
        actions: [
          IconButton(
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              userPreferences.updateHouseType(null);
              userPreferences.updateRentalDetails(
                  selfContained: null, fenced: null);
              userPreferences.updateAirbnbDetails(amenities: {});
            },
          ),
        ],
      ...
      body: StreamBuilder<List<Property>>(
        ...
        builder: (context, snapshot) {
          ...
          final displayedProperties = _applyClientSideFiltersAndScore(
              firebaseProperties, userPreferences);
          ...
          // Show map view or list view based on _isMapView state
          if (_isMapView) {
            return SimpleMapView(
              ...
              userLatitude: _appliedReferenceLatitude,
              userLongitude: _appliedReferenceLongitude,
            );
          } else {
